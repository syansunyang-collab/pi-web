...
const { AgentSessionWrapper, canonicalSessionPath, registerRpcWrapper, startRpcSession } = await jiti.import("./rpc-manager.ts");

function makeRegisteredInner(sessionId, sessionFile) {
  return {
    sessionId,
    sessionFile,
    isBashRunning: false,
...
test("canonicalSessionPath folds path aliases, and letter case on Windows", () => {
  const dir = join(tmpdir(), "pi-web-canonical");
  const straight = join(dir, "session.jsonl");
  const aliased = join(dir, "..", "pi-web-canonical", ".", "session.jsonl");

  const canonical = canonicalSessionPath(aliased);
  assert.equal(canonical, canonicalSessionPath(straight));
  if (process.platform === "win32") {
    assert.equal(canonical, canonical.toLowerCase());
    assert.equal(canonicalSessionPath(straight.toUpperCase()), canonical);
  }
});

test("RPC registry deduplicates live wrappers by canonical session file path", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");

  assert.match(source, /var __piSessionPathIndex: Map<string, string> \| undefined/);
  assert.match(source, /const canonicalPath = sessionFile \? canonicalSessionPath\(sessionFile\) : undefined/);
  assert.match(source, /const lockKey = canonicalPath \?\? sessionId/);
  assert.match(source, /function getLivePathOwner\(canonicalPath: string\)/);
  assert.match(source, /canonicalSessionPath\(indexed\.sessionFile\) === canonicalPath/);
  assert.match(source, /const indexed = getLivePathOwner\(canonicalPath\)/);
  assert.match(source, /if \(indexed && indexed\.sessionId !== sessionId\)/);
  assert.match(source, /currentOwner && currentOwner !== wrapper/);
  assert.match(source, /locks\.set\(lockKey, starting\)/);

  // The dedup check must run before SessionManager.open: an already-owned
  // file must never be opened a second time just to discover the owner.
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const dedupCheck = startupSource.indexOf("getLivePathOwner(canonicalPath)");
  const writableOpen = startupSource.indexOf("SessionManager.open(sessionFile");
  assert.ok(dedupCheck >= 0 && writableOpen > dedupCheck);
});

test("startRpcSession returns the live wrapper already owning the canonical file path", async (t) => {
  // The alias never reaches the filesystem: the dedup check runs before
  // SessionManager.open, so an uncreated path is enough to exercise it.
  const dir = join(tmpdir(), "pi-web-rpc-dedup", "sessions");
  const filePath = join(dir, "owned.jsonl");
  const aliasedPath = `${dir}${sep}nested${sep}..${sep}owned.jsonl`;
  const lookupPath = process.platform === "win32" ? aliasedPath.toUpperCase() : aliasedPath;

  const owner = new AgentSessionWrapper(makeRegisteredInner("real-id", filePath), { chatOnly: true });
  t.after(() => owner.destroy());
  registerRpcWrapper(owner);

  const result = await startRpcSession("alias-id", lookupPath, undefined);

  assert.equal(result.session, owner);
  assert.equal(result.realSessionId, "real-id");
});

test("a replaced wrapper's destroy does not evict its successor from the registry or path index", async (t) => {
  const dir = join(tmpdir(), "pi-web-rpc-dedup", "sessions");
  const filePath = join(dir, "rebuilt.jsonl");
  const aliasedPath = `${dir}${sep}nested${sep}..${sep}rebuilt.jsonl`;

  const stale = new AgentSessionWrapper(makeRegisteredInner("same-id", filePath), { chatOnly: true });
  registerRpcWrapper(stale);
  const successor = new AgentSessionWrapper(makeRegisteredInner("same-id", filePath), { chatOnly: true });
  t.after(() => successor.destroy());
  registerRpcWrapper(successor);

  // The stale wrapper's deferred dispose must not remove entries the
  // successor registered under the same id and canonical path.
  stale.destroy();

  const result = await startRpcSession("alias-id", aliasedPath, undefined);
  assert.equal(result.session, successor);
  assert.equal(result.realSessionId, "same-id");
});

test("a stale path entry self-evicts once the indexed wrapper owns a different file", async (t) => {
  const dir = join(tmpdir(), "pi-web-rpc-dedup", "sessions");
  const oldPath = join(dir, "old.jsonl");
  const newPath = join(dir, "new.jsonl");
  const aliasedOldPath = `${dir}${sep}nested${sep}..${sep}old.jsonl`;

  const moved = new AgentSessionWrapper(makeRegisteredInner("moved-id", oldPath), { chatOnly: true });
  t.after(() => moved.destroy());
  registerRpcWrapper(moved);
  // The session now lives in a different file; the old path entry is stale
  // and must not route lookups for it to this wrapper.
  moved.inner.sessionFile = newPath;

  const next = new AgentSessionWrapper(makeRegisteredInner("next-id", oldPath), { chatOnly: true });
  t.after(() => next.destroy());
  registerRpcWrapper(next);

  const result = await startRpcSession("alias-id", aliasedOldPath, undefined);
  assert.equal(result.session, next);
  assert.equal(result.realSessionId, "next-id");
});

test("registration does not steal the path index from a live owner", async (t) => {
  const dir = join(tmpdir(), "pi-web-rpc-dedup", "sessions");
  const filePath = join(dir, "contested.jsonl");
  const aliasedPath = `${dir}${sep}nested${sep}..${sep}contested.jsonl`;

  const owner = new AgentSessionWrapper(makeRegisteredInner("owner-id", filePath), { chatOnly: true });
  t.after(() => owner.destroy());
  registerRpcWrapper(owner);

  // A directly registered wrapper for the same file must not take over the
  // index while the current owner is alive; alias lookups keep deduping to
  // the owner instead of opening the file a third time.
  const intruder = new AgentSessionWrapper(makeRegisteredInner("intruder-id", filePath), { chatOnly: true });
  t.after(() => intruder.destroy());
  registerRpcWrapper(intruder);

  const result = await startRpcSession("alias-id", aliasedPath, undefined);
  assert.equal(result.session, owner);
  assert.equal(result.realSessionId, "owner-id");
});
