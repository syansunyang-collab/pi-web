...
test("configures HTTP_PROXY, HTTPS_PROXY, and NO_PROXY for global fetch", async (t) => {
  ...
  assert.deepEqual(directRequests, ["GET http://target.invalid/through-http-proxy"]);
  ...
  const proxiedRequestCount = directRequests.length + connectTargets.length;
  await assert.rejects(fetch("http://bypass.invalid:9/no-proxy", {
    signal: AbortSignal.timeout(2_000),
  }));
  assert.equal(directRequests.length + connectTargets.length, proxiedRequestCount);
});
